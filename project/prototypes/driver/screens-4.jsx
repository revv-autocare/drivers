/* global React, useStore, findShop, BackArrow, BottomNav, DetailHead, Pill */
const { useState: useS4, useMemo: useM4 } = React;

// ═══════════ FIND A SHOP ══════════════════════════════════════════
function FindShopScreen({ go }) {
  const { shops } = useStore();
  const [query, setQuery] = useS4('');
  const [filter, setFilter] = useS4('all'); // all | nearby | top-rated | verified
    ...
    <div className="dv-screen">
      ...
      <div style={{ padding: '14px 0 0', background: '#fff', borderBottom: '1px solid var(--border-subtle)' }}>
        <div className="dv-searchbar" style={{ marginBottom: 10 }}>
          ...
          <input placeholder="Shop name, neighborhood, specialty…" value={query} onChange={e => setQuery(e.target.value)}/>
        </div>
        <div className="dv-chips">
          {[
            ...
            <div key={f.id} className={'dv-chip ' + (filter === f.id ? 'on' : '')} onClick={() => setFilter(f.id)}>
              {f.label}
            </div>
          ))}
        </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  {s.verified && <svg width="13" height="13" viewBox="0 0 24 24" fill="var(--color-brand-500)"><path d="M12 2 4 5v6c0 5 3.4 9.7 8 11 4.6-1.3 8-6 8-11V5l-8-3zm-1 14-4-4 1.4-1.4L11 13.2l5.6-5.6L18 9l-7 7z"/></svg>}
                ...
                <div style={{ fontSize: 12, color: 'var(--fg-secondary)', marginTop: 2, display: 'flex', alignItems: 'center', gap: 8 }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }}>
                    <svg width="11" height="11" viewBox="0 0 24 24" fill="var(--color-warning-500)"><path d="m12 2 3.1 6.3 7 1-5 4.9 1.1 6.8L12 18l-6.2 3.3 1.2-6.8-5-5 7-1z"/></svg>
...
// ═══════════ SHOP DETAIL + BOOKING ════════════════════════════════
function ShopDetailScreen({ go, shopId }) {
    ...
    <div className="dv-screen" style={{ background: '#fff' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
              ...
              {shop.verified && <svg width="16" height="16" viewBox="0 0 24 24" fill="var(--color-brand-500)"><path d="M12 2 4 5v6c0 5 3.4 9.7 8 11 4.6-1.3 8-6 8-11V5l-8-3zm-1 14-4-4 1.4-1.4L11 13.2l5.6-5.6L18 9l-7 7z"/></svg>}
            </div>
            <div style={{ fontSize: 13, color: 'var(--fg-secondary)', marginTop: 2 }}>{shop.neighborhood} · {shop.distanceKm} km</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 6 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="var(--color-warning-500)"><path d="m12 2 3.1 6.3 7 1-5 4.9 1.1 6.8L12 18l-6.2 3.3 1.2-6.8-5-5 7-1z"/></svg>
      ...
      <div style={{ padding: '18px 16px', flex: 1, overflowY: 'auto' }}>
        ...
        <div className="dv-vincard" style={{ marginBottom: 16 }}>
          <div className="info-row" style={{ borderTop: 'none' }}><span className="l">Address</span><span className="v" style={{ textAlign: 'right', maxWidth: '60%' }}>{shop.address}</span></div>
          <div className="info-row"><span className="l">Hours</span><span className="v">{shop.hours}</span></div>
          <div className="info-row"><span className="l">Honor rate</span><span className="v" style={{ color: 'var(--color-success-700)' }}>{shop.honorRate}% · responds within 24h</span></div>
        </div>
      ...
      <div className="dv-actbar">
        <button className="dv-btn dv-btn--ghost-icon" onClick={() => alert('Calling shop...')}>
...
// ═══════════ BOOK APPOINTMENT ═════════════════════════════════════
function BookAppointmentScreen({ go, shopId }) {
  ...
  const [service, setService] = useS4(SERVICES[0]);
  const [day, setDay] = useS4(DAYS[1].iso);
  const [time, setTime] = useS4(TIMES[2]);
  const [notes, setNotes] = useS4('');
  const [booking, setBooking] = useS4(false);
  ...
  return (
    <div className="dv-screen" style={{ background: '#fff' }}>
      ...
      <div style={{ padding: '16px', flex: 1, overflowY: 'auto' }}>
        ...
        <h4 style={{ margin: '0 0 10px', fontSize: 13, fontWeight: 600 }}>Notes for the shop <span style={{ fontWeight: 400, color: 'var(--fg-tertiary)' }}>(optional)</span></h4>
...
Object.assign(window, { FindShopScreen, ShopDetailScreen, BookAppointmentScreen, BookingConfirmScreen });
